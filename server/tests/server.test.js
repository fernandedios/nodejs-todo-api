...
describe('GET /todos/:id', () => {
  ...
  it('should not return todo doc created by other user', (done) => {
    request(app)
      .get(`/todos/${todos[1]._id.toHexString()}`) // toHexString() convert ObjectID to String
      .set('x-auth', users[0].tokens[0].token)
      .expect(404)
      .end(done);
  });
...
describe('DELETE /todos/:id', () => {
  ...
  it('should not remove a todo if owned by another user', (done) => {
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        Todo.findById(hexId) // check if todo still exists
          .then((todo) => {
            expect(todo).toExist();
            done();
          })
  ...
  it('should reject invalid login', (done) => {
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        User.findById(_id)
          .then((user) => {
            expect(user.tokens.length).toBe(1); // 1 since both users already have pre-existing tokens
            done();
          })
          .catch((err) => done(err));
      });
